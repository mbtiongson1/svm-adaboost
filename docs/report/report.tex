...
Binary classification of non-linearly separable data is one of the more instructive problems in machine learning—it forces algorithms to capture structure that no single linear boundary can represent. Two families of methods tackle this in fundamentally different ways: ensemble methods that combine many simple linear classifiers into a collectively expressive hypothesis, and kernel-based methods that geometrically reshape the problem so that a linear separator suffices.
...
Support Vector Machines (SVMs), developed by Vapnik and colleagues~\cite{cortes1995}, take a geometrically elegant approach: find the hyperplane that maximizes the margin between classes, then use the kernel trick to operate implicitly in a high-dimensional feature space---enabling curved decision boundaries in the original input space without ever constructing the transformation explicitly.

This work pits these two paradigms against each other: AdaBoost with Pocket Algorithm weak learners implemented from scratch in NumPy, versus scikit-learn's \texttt{SVC}, on the Banana dataset (a two-dimensional problem with crescent-shaped class regions) and the Splice dataset (a 60-dimensional biological sequence classification task).
...
At each of $T$ iterations, a training example $(\mathbf{x}_j, y_j)$ is drawn uniformly at random. The sign function used throughout is $\text{sgn}(z) = +1$ if $z \geq 0$ and $-1$ otherwise. If $\mathbf{v}$ classifies $\mathbf{x}_j$ correctly---that is, $\text{sgn}(\mathbf{v}^T \tilde{\mathbf{x}}_j) = y_j$---the streak counter $n_v$ increments. On a misclassification, if the current streak $n_v$ exceeds the pocket record $n_w$, the pocket is updated: $\mathbf{w} \leftarrow \mathbf{v}$, $n_w \leftarrow n_v$. The standard Perceptron correction is then applied,
\begin{equation}
...
On Banana, the RBF SVM edges out the Boosted Perceptron by under one percentage point (90.14\% vs.\ 89.18\% at $K=1000$). That the gap is so narrow is arguably more interesting than the outcome itself: an ensemble of 1000 linear classifiers comes within one point of a kernel that implicitly operates in infinite dimensions.

On Splice the story changes. A 6-point gap (89.15\% vs.\ 83.17\%) is not noise---it reflects a genuine structural advantage. In 60 dimensions, individual linear cuts can only carve out so much signal; the RBF kernel's ability to model complex, non-linear feature interactions gives it a decisive edge, while the AdaBoost ensemble, despite hitting 100\% training accuracy, fails to generalize comparably.

\subsection{Overfitting Behavior}

Splice AdaBoost is a textbook case of memorization: training accuracy climbs monotonically to 100\% by round 800 while test accuracy stagnates around 83\%. Each additional weak learner is fitting idiosyncrasies of the resampled bootstrap distribution rather than generalizable structure. Banana fares better---a 3.5-point train--test gap at $K=1000$ compared to nearly 17 points on Splice---largely because the simpler two-dimensional geometry leaves less spurious signal to chase.

The SVM also achieves 100\% training accuracy on Splice, yet its test accuracy is 89.15\%---substantially higher. The maximum-margin principle acts as an implicit regularizer by pushing the decision boundary as far as possible from the nearest training points; AdaBoost with Pocket Algorithm base learners has no equivalent structural bias to resist memorization.
...
This work pitted two principled approaches to non-linear binary classification against each other---Boosted Perceptrons built from Pocket Algorithm weak learners, and RBF-kernel SVMs---across datasets that stress-test different aspects of each method. The key findings are:
...
Together, these results suggest that maximum-margin geometry---even with an off-the-shelf library---can match or outperform a carefully designed boosting ensemble at a fraction of the training cost, particularly when the feature space is rich and training data is limited.
